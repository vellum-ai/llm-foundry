...
from llmfoundry.callbacks.save_peft_callback import SavePeftCallback
...
def validate_config(cfg: DictConfig):
    ...
    if lora_config is not None and isinstance(lora_config, (dict, DictConfig)):
        ...
        if args is not None and isinstance(args, (dict, DictConfig)):
            ...
            if target_modules is None or not isinstance(
                target_modules, (list, ListConfig)
            ):
...
def build_dataloader(
    cfg: DictConfig, tokenizer: PreTrainedTokenizerBase, device_batch_size: int
):
...
def main(cfg: DictConfig):
    ...
    fsdp_dict_config: Optional[DictConfig] = pop_config(
        cfg, "fsdp_config", must_exist=False, default_value=None
    )
    fsdp_config: Optional[Dict] = (
        om.to_container(fsdp_dict_config) if fsdp_dict_config is not None else None
    )  # type: ignore

    eval_loader_config: Optional[DictConfig] = pop_config(
        cfg, "eval_loader", must_exist=False, default_value=None
    )
    icl_tasks_config: Optional[ListConfig] = pop_config(
    ...
    python_log_level: str = pop_config(
    ...
    if eval_loader_config is not None:
        ...
        eval_dataloader = build_dataloader(
            eval_loader_config, tokenizer, device_eval_batch_size
        )
        eval_metric_names = list(model.train_metrics.keys())
        eval_loader = Evaluator(
            label="eval", dataloader=eval_dataloader, metric_names=eval_metric_names
        )
        evaluators.append(eval_loader)

    if icl_tasks_config is not None:
        icl_evaluators, _ = build_icl_evaluators(
            icl_tasks_config, tokenizer, max_seq_len, device_eval_batch_size
        )
    ...
    callbacks.append(SavePeftCallback(save_folder))
